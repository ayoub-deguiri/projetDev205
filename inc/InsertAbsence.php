...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["valider"])) {
    ...
    foreach ($result as $cef) {
        $error = 1;
        $type = "";
        if (!empty($_GET['absence-' . $cef[0]])) {
            $type = 'absence';
            $error = 0;
        }
        if (!empty($_GET['retard-' . $cef[0]])) {
            $type = 'retard';
            $error = 0;
        }
        if ($error == 0) {
            ...
            $timeDebut = $_GET['debut-' . $cef[0]];
            $timeFin = $_GET['Fin-' . $cef[0]];
            ...
            $pdo_statement->execute();
            header('location:./../responsable.php?successmsg=les absence et bien enregistré');
        }
    }
